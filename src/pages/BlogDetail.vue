<template>
  <div>
    <PageHeader
      ...
      :title="listBlogs[indexBLog].title"
      className="post-heading"
      :subheading="cutPara"
    >
      <span class="meta"
        >Posted by
        <a href="#">{{listBlogs[indexBLog].fullName}}</a>
        on {{formatDate}}</span
      >
    </PageHeader>
    <article>
        ...
        <div class="row">
          <div class="col-lg-8 col-md-10 mx-auto">
            ...
            <a href="#">
              <img
                class="img-fluid"
                src="/dist/img/post-sample-image.jpg"
                alt=""
              />
            </a>
...
export default {
  ...
  computed: {
    ...mapState(["listBlogs"]),
    indexBLog() {
      return this.listBlogs.findIndex((item,index)=>{
         return this.$route.params.id === item.id;
      })
    },
    formatDate() {
      var date = null;
      this.listBlogs.forEach(element => {
        if(this.$route.params.id === element.id)  date = element.create_at;
    ...
    cutPara(){
      let arrAbs = [];
      this.listBlogs.forEach(e=>{
        if(this.$route.params.id === e.id){
         arrAbs.push(e.abstract.split(' '))
        }
      })
      arrAbs[0].length = 10;
      // arrAbs[0].join();
      return arrAbs[0].join(' ');
      console.log( arrAbs[0].join(' '));
    }
  },
